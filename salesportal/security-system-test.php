...
<div id="portal_container" class="logged_in">
    ...
    <section id="security_system" class="page">
        ...
        <div id="package_modules">
            <?php require_once 'modules/package-details-test.php' ?>
            <?php require_once 'modules/payment-info-test.php' ?>
        ...
        <input id="confirm-test" type="button" class="button" value="Confirm (Validation Test)"/>
...
<script>
    ...
    $(document).ready(function () {
        ...
        $('#confirm-test').click(function() {
            ...
            if ($('#checking_account_select').is(':checked') && $('#routing_number').val().length !== 9) {
                errorMessage += 'The routing number must have 9 digits. ';
                errors += 1;
            }

            if ($('#credit_card_number').val().toString().charAt(0) === '3') {

                if ($('#credit_card_select').is(':checked') && $('#credit_card_number').val().length !== 15) {
                    errorMessage += 'American Express cards must have 15 digits. ';
                    errors += 1;
                }

            } else {

                if ($('#credit_card_select').is(':checked') && $('#credit_card_number').val().length !== 16) {
                    errorMessage += 'The credit card number must have 16 digits. ';
                    errors += 1;
                }

            }

            if ($('#state input').val() == 'PA' || $('#state input').val() == 'CA') {
                if ($('#additional_notes textarea').val() == '') {
                    errorMessage += 'The Additional Notes section must be filled out. ';
                    errors += 1;
                }
            }
            ...
            if (errors > 0) {
                if (errors == 1) {
                    $('#form-errors span').html(errors + ' error');
                } else {
                    $('#form-errors span').html(errors + ' errors');
                }
                $('#form-errors p').html(errorMessage);
                $('#modal_container, #form-errors').css('display', 'flex').hide().fadeIn();
