<?php
    ...
    $sql = $dbConn->prepare("SELECT JOB_ID, SCHEDULED_JOBS.USER_ID, USR.USER_FIRST_NAME, USR.USER_LAST_NAME, USR.USER_ZIP, SCHEDULED_DATE_TIME, DATA_1, DATA_2, DATA_3, ZIP, SCHEDULED_JOBS.SERVICE_ID, TIMEOFF_ID, END_DATE_TIME FROM SCHEDULED_JOBS JOIN USR ON SCHEDULED_JOBS.USER_ID = USR.USER_ID WHERE SCHEDULED_JOBS.COMPANY_ID = ? AND SCHEDULED_DATE_TIME >= ? AND SCHEDULED_DATE_TIME <= ?");
...
<section id="calendar" class="page">
    ...
    <div id="week" class="calendar-view table">
        ...
        <div class="table-headings">
            <div class="tech-name">Technician</div>
            <div>Monday 00</div>
            <div>Tuesday 00</div>
...
<script>
    ...
    $(document).ready(function(){
        ...
        $("#week").on('click', '.event', function(){
            const url = 'https://crm.zoho.com/crm/EntityInfo.do' + $(this).data().querystring;
        ...
        function putJobsOnSchedule(jobs){
            ...
            _.each(techs, function(jobs, userId) {
                ...
                if (jobs[0].USER_ZIP === null) {
                    techZip = '';
                } else {
                    techZip = '<div class="tech-zip">' + jobs[0].USER_ZIP + '</div>';
                }

                $('#week').append(
                    '<div class="table-row">' +
                    '<div class="table-cell tech-name">' +
                    '<a href="edit-user.php?userId=' + userId + '">' + jobs[0].USER_FIRST_NAME + '<br>' + jobs[0].USER_LAST_NAME  + techZip + '</a>' +
                    '</div>' +
                    '</div>'
                );
                ...
                for (i=0; i < 7; i++){
                    ...
                    if (jobs.length > 0) {
                        ...
                        jobs = _.sortBy(jobs, function(job) {
                            return job.SCHEDULED_DATE_TIME;
                        });
                        ...
                        _.each(jobs, function(job) {
                            ...
                            if (job.SERVICE_ID) {
                                ...
                                queryString = '?module=CustomModule9&id=' + job.SERVICE_ID;
                            } else {
                                iconClass = 'icon-home';
                                queryString = '?module=Potentials&id=' + job.JOB_ID;
                            }

                            if (job.TIMEOFF_ID) {
                                iconClass = 'icon-cross';
                                queryString = '?module=CustomModule10&id=' + job.TIMEOFF_ID;
                                timeOffStartTime = moment(job.SCHEDULED_DATE_TIME).format('h:mm A');
                                timeOffEndTime = moment(job.END_DATE_TIME).format('h:mm A');
                                scheduledTime = timeOffStartTime + ' -<br>' + timeOffEndTime;
                            }
                            ...
                            if (iterationDate.isSame(scheduledDate)) {
                                ...
                                $('#week').find('.table-row:last-child .table-cell:last-child').append(
                                    '<div class="event" data-querystring="' + queryString + '">' +
                                    '<i class="event-type ' + iconClass + '"></i>' +
                                    '<div class="event-info">' +
                                    '<div class="event-time">' + scheduledTime + '</div>' +
                                    '<div class="event-customer" title="' + (job.DATA_1).toLowerCase() + '">' + (job.DATA_1).toLowerCase() + '</div>' +
                                    '<div class="event-customer-zip">' + customerZip + '</div>' +
                                    '</div>' +
                                    '</div>'
                                );
                            }
