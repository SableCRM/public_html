<?php
    ...
    $sql = $dbConn->prepare("SELECT JOB_ID, SCHEDULED_JOBS.USER_ID, USR.USER_FIRST_NAME, USR.USER_LAST_NAME, SCHEDULED_DATE_TIME, DATA_1, DATA_2, DATA_3, SCHEDULED_JOBS.SERVICE_ID FROM SCHEDULED_JOBS JOIN USR ON SCHEDULED_JOBS.USER_ID = USR.USER_ID WHERE SCHEDULED_JOBS.COMPANY_ID = ? AND SCHEDULED_DATE_TIME >= ? AND SCHEDULED_DATE_TIME <= ?");
...
<script>
    ...
    $(document).ready(function(){
        ...
        $("#week").on('click', '.event', function(){
            window.open('https://crm.zoho.com/crm/EntityInfo.do?module=Potentials&id=' + $(this).data().dealid, '_blank');
        });
